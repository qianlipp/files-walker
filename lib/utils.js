...
var canThroughFilter = function (options, path, filename) {
    ...
    if (options.contain != undefined) {
        return (path + filename).indexOf(options.contain) > -1;
    }
    if (options.not_contain != undefined) {
        return (path + filename).indexOf(options.not_contain) == -1;
    }
    if (options.start_with != undefined) {
        return isStartWith(path + filename, options.start_with);
    }
    if (options.not_start_with != undefined) {
        return !isStartWith(path + filename, options.not_start_with);
    }
    if (options.end_with != undefined) {
        return isEndWith(path + filename, options.end_with);
    }
    if (options.not_end_with != undefined) {
        return !isEndWith(path + filename, options.not_end_with);
    }
